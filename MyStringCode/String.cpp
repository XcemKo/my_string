...
//a - 097 : z - 122
//0 - 048 : 9 - 057
int lowerCase(int src) {
	return ((src >= 65 && src <= 90) || (src >= 192 && src <= 223)) ? src + 32 : src;
}
...
bool operator>(const Str& left, const Str& right) {
	...
	if (left != right) {
		...
		for (int i = 0;i<min_size + 1;i++) {
			unsigned char tmp_l = lowerCase(left.source[i]);
			unsigned char tmp_r = lowerCase(right.source[i]);
			if (tmp_l<tmp_r)
			{
				result = true;
				break;
